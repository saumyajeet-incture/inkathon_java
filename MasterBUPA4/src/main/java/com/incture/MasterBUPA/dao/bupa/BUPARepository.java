...
//	Page<BusinessPartner> getBasicDetailsByFirstName(String firstName,Pageable pageable);
	...
	String query4="SELECT * from BUSINESS_PARTNER where LOWER(FIRST_NAME) LIKE ?1%";
	@Query(value = query4, nativeQuery = true)
	Page<BusinessPartner> getBasicDetailsByFirstName(String firstName,Pageable pageable);
	
	String query5="SELECT * from BUSINESS_PARTNER where LOWER(LAST_NAME) LIKE ?1%";
	@Query(value = query5, nativeQuery = true)
	Page<BusinessPartner> getBasicDetailsByLastName(String lastName, Pageable pageable);
	
	String query6="SELECT * from BUSINESS_PARTNER where LOWER(BP_ROLE) LIKE ?1%";
	...
	String query10= "SELECT * from BUSINESS_PARTNER where BP_ID=?1 AND LOWER(BP_ROLE)=?2";
	@Query(value = query10, nativeQuery = true)
	Page<BusinessPartner> getBasicDetailsByBpIdBpRole(int bp_id,String bpRole,Pageable pageable);
	
	String query11= "SELECT * from BUSINESS_PARTNER where ROLE_ID=?1 AND LOWER(BP_ROLE)=?2";
	@Query(value = query11, nativeQuery = true)
	Page<BusinessPartner> getBasicDetailsByRoleIdBpRole(int role_id,String bpRole,Pageable pageable);
